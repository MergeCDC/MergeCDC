...
void Graph::Convert2Graph() {
    ...
    for(int i = 0; i < nVertices; i++) {
        ...
        for(int j = 0; j < nVertices; j++) {
            ...
            int wgt = fileDist->GetSimilarity(blockInfo[i], blockInfo[j]) * 100; // 保留两位小数
            if(wgt != 0) {
                adjncy.push_back(j);
...
void METISCombiner::CombineNodes() {
    struct timeval start, end;
    double rTime;
    gettimeofday(&start,NULL);
    idx_t nVertices = graph->xadj.size() - 1, nEdges = graph->adjncy.size() / 2, nWeights = 1, objval;
    parts.resize(nVertices);
    // int ret = METIS_PartGraphRecursive(&nVertices, &nWeights, graph->xadj.data(), graph->adjncy.data(), graph->vwgt.data(), NULL, graph->adjwgt.data(), &nParts, NULL, NULL, NULL, &objval, parts.data());
    int ret = METIS_PartGraphKway(&nVertices, &nWeights, graph->xadj.data(), graph->adjncy.data(), graph->vwgt.data(), NULL, graph->adjwgt.data(), &nParts, NULL, NULL, NULL, &objval, parts.data());
    // int ret = METIS_PartGraphKway(&nVertices, &nWeights, graph->xadj.data(), graph->adjncy.data(), NULL, NULL, graph->adjwgt.data(), &nParts, NULL, NULL, NULL, &objval, parts.data());
    if(ret != METIS_OK) {
        cerr << "combine nodes error, err: " << ret << endl;
    }
    gettimeofday(&end,NULL);
    rTime = (end.tv_sec*1000000.0 + end.tv_usec -
		 	start.tv_sec*1000000.0 - start.tv_usec) / 1000000.0;
    cout << "combination time: " << rTime << endl; 
    vector<vector<int>> combRes(nParts);
    for(int i = 0; i < parts.size(); i++) {
        combinedDist[parts[i]].insert(combinedDist[parts[i]].end(), graph->blockInfo[i].begin(), graph->blockInfo[i].end());
        combRes[parts[i]].push_back(i);
        for(int j = 0; j < graph->blockInfo[i].size(); j++) {
            fileRealNodeTab[parts[i]][graph->blockInfo[i][j]].push_back(i);
        }
    }
    cout << endl << "file storage: " << endl;
    for(int i = 0; i < nParts; i++) {
        for(int j = 0; j < combRes[i].size(); j++) {
            for(int k = 0; k < graph->blockInfo[combRes[i][j]].size(); k++) {
                cout << graph->blockInfo[combRes[i][j]][k] << " ";
            }
            cout << endl;
        }
    }
    ofstream rofs("result", ios::app);
    ...
    ofstream ofs("combination_metis");
    if(!ofs) {
        cerr << "fail to open file combination_m" << endl;
        assert(false);
    }
    for(int i = 0; i < nVertices; i++) {
        ofs << parts[i] << " ";
...
void METISCombiner::GenRandomDist() {
    combinedDist.clear();
...
void METISCombiner::GenMyCombinationDist() {
    struct timeval start, end;
    double rTime;
    combinedDist.clear();
    combinedDist.resize(nParts);
...
void METISCombiner::GenDFSCombinationDist() {
    struct timeval start, end;
    double rTime;
    combinedDist.clear();
    combinedDist.resize(nParts);
...
void METISCombiner::SearchForGroups() {
...
int METISCombiner::CalCombination(int m, int n) { // calculate m choose n
    long long res = 1;
    for(int i = 0; i < n; i++) {
        res *= m - i;
    }
